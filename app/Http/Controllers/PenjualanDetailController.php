...
class PenjualanDetailController extends Controller
{
    public function index()
    {
        ...
        // Cek apakah ada transaksi penjualan yang sedang berjalan ?
         if ($id_penjualan = session('id_penjualan')) {
            $penjualan = Penjualan::find($id_penjualan);
            $memberSelected = $penjualan->member ?? new Member();

            return view('penjualan_detail.index', 
                compact('produk', 'member', 'diskon', 
                        'id_penjualan', 'penjualan', 'memberSelected'));
        } else {
    ...
    public function loadform($diskon = 0, $total, $diterima)
    {
        ...
        $data    = [
            ...
            'bayar_terbilang' => ucwords(terbilang($bayar). ' Rupiah'),
